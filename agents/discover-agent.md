...
```bash
qua-search books --author "{full_name}" --limit 20
qua-search papers --author "{full_name}" --limit 30
```
...
```
qua-search: books={n_books}, papers={n_papers}
```
...
```bash
qua-search validate --manifest {manifest_path}
```
