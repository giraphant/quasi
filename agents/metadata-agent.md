...
`observations` 是精炼证据记录，`issue` 是阶段判断而不是 CLI stderr 转储。凭据、signed URL
和 shell command 不进入 receipt。
