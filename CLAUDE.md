...
- Skills drive. For Paper, Book, Talk, and Translation, `collect-material` observes one exact status, selects the fixed named entry by kind, consumes one material-level result, presents a typed gate when needed, and re-observes exact artifacts after completion. Archive and Webpage URL intake transport the exact public URL with `observation:null`, then observe the canonical route returned by their fixed entry before resuming. Explicit archival intent uses Archive; standalone webpage reading uses Webpage. Topic web tasks discover exact sources, compose Archive collection with fresh observations, then write cards referencing those Archives. One named invocation owns one logical material; only Book may use the host `pipeline()` internally for disjoint chapter outputs. Author and Topic have named composition entries: they return exact child or Topic routes for host observation, then resume from that fresh testimony without rediscovery. `research-topic` only transports the Topic envelope, typed gates, and exact observations.
...
workflows/paper.mjs | book.mjs | talk.mjs | translation.mjs | author.mjs | topic.mjs | webpage.mjs | archive.mjs
