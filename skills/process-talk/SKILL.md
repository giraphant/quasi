...
- `quasi-helpers talk compress-media --media F --output O` → 本机压缩单个录制。
...
1. **Step 0** — 计算 `slug`。若 `vault/talks/{slug}/talk.md` 已存在且 audit 干净,
   ...
   `vault/talks/{slug}/` 存在,确认 `.gitignore` 覆盖 `recording.*`。
2. **Step 1a** — 若输入是视频,先本机压缩:
   `quasi-helpers talk compress-media --media {media} --output vault/talks/{slug}/recording.mp4`,
   后续 `media` 改用该输出。音频文件跳过。
3. **Step 1** — `quasi-transcribe run --media … --slug … --title …`。`ok=false`
   (全引擎空)→ 视作 dead 候选,转 Step 2 的 dead 分支(用户可换源)。
4. **Step 2** — `quasi-transcribe classify --slug …`。`dead/empty` → `quasi-transcribe
   silent …` 写模板 `talk.md`,转 Step 4。`live` → Step 3。
5. **Step 3** — 一次 `analyse-agent`(`type: T`),传 `slug`、`transcript_path`、
   `per_engine` SRT 路径、`title`、`date`、`media`、`output`(绝对路径)。它按 agent 的
   `<talk_mode>` 交叉比对三份转写,写 `talk.md`(六个四字 H2,`时间脉络` 必填)。
6. **Step 4** — `audit-agent`。诊断 `escalated` → 让 `analyse-agent` 按诊断再生成一次
   → 复审;仍 escalated 则报告并停。
7. **Step 5** — best-effort `marple` 打开 `vault/talks/{slug}/talk.md`。
