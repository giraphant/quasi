#!/usr/bin/env python3
"""quasi-helpers citation — deterministic citation helpers for quasi drafts.
...
def cmd_biblio(args) -> int:
    root = _project_root(args.project_root)
    data = biblio_mod.scan_vault(root)
    out = Path(args.output)
    write_json(out, data)
    s = data["summary"]
    print(
...
def main(argv: list[str]) -> int:
    ...
    p_bib = sub.add_parser("biblio", help="Scan vault frontmatter → biblio.json")
    ...
    p_res.add_argument("--biblio", required=True, help="biblio.json (from `quasi-helpers citation biblio`)")
