...
async function validateRuntimeBundle(source) {
  ...
  if (/from\s+["']node:fs|["']fs["']/.test(source))
    throw new Error("workflow bundle must not contain fs access");
  const module = await import(
    `data:text/javascript;base64,${Buffer.from(source).toString("base64")}`
  );
  const expectedExports = ["meta", ...WORKFLOW_EXPORTS].sort();
  const actualExports = Object.keys(module).sort();
  if (JSON.stringify(actualExports) !== JSON.stringify(expectedExports))
    throw new Error(
      `workflow bundle exports ${actualExports.join(", ")}; expected ${expectedExports.join(", ")}`,
    );
}
