...
Codex 按滚雪球法组织研究：从关键材料和已有笔记出发，追索人物、事件、引用、矛盾与空白，阅读新材料后回看笔记，形成解释并带出下一步搜索。主代理判断哪些分支值得深入、横向比较或暂时搁置，并在原有来源圈之外寻找独立记录。CC 启动、路由与验收放在材料处理时才读取的参考文件中。CC 按一件完整材料执行 `collect-material`，或对明确 draft 执行 `finalise-draft`。一项材料内部的 Workflow、观察和审计不回传给 Codex 逐阶段驱动。
...
用户指定的研究轮次约束探索预算，每轮从有来由的线索出发，可包含多次检索和多件材料，下一轮承接本轮的问题；数量由证据价值决定。主代理在采集期间继续独立研究，阶段小结后继续完成用户指定轮数；单个 worker 完成或材料受阻不会结束整项研究。
...
5. 多轮研究应能看出实际线索如何带来新材料、笔记如何带来新问题，以及新证据如何影响判断。评估这种连续探索、旧笔记回读和来源多样性，不按新增档案数证明轮次。
