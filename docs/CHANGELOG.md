...
- **0.68.0** (2026-09-26): Replace DS OCR2 recognition plus separate MinerU grouping with direct MinerU2.5-Pro `two_step_extract`, pinned to mineru-vl-utils 2.0.5 / mlx-vlm 0.7.3 / mlx 0.32.2. One model load serves each foreground invocation. Normal OCR preserves reading-order text, notes, tables and formulas; layout places complete paragraphs over unchanged scan images, keeps furniture separate and preserves table/formula images. Unsupported/missing recognition and placement errors propagate without automatic Tesseract or per-line fallback.
  - Carry source/model/text-bound disagreement evidence in both PDFs and MinerU generation manifests. Keep new text, flag body agreement below 0.85, distinguish absent baselines and empty pages, normalize comparison-only hyphens, and exempt classified notes plus source-confirmed dense reference paragraphs. Fit font size to each paragraph box with body-only size snapping; clean supported LaTeX/HTML without dropping characters. Bind the model/revision into generation keys and new layout evidence. Historical DS generations reconcile read-only; partial DS work cannot be continued by MinerU. Tesseract remains explicit, including compatible legacy resume.
  - Release validation: full suite 1,827 passed / 1 skipped (225.63s). Workflow build/parity/type checks, plugin and marketplace manifest validation, and mirrored instructions passed. The skipped paid Host E2E was not run. Real 38-page local inference completed in 133.33 seconds; both output modes retain all recognized characters, and all scan-image streams are unchanged. Harrington p31's missing lines return; seven known glued-word pages have no long joined tokens; all four p215 suspect paragraphs remain flagged. The fifth flagged paragraph has a garbled old baseline. Barnes 178-page recognition completed in 765.24 seconds, with no recognized-character loss after glyph-coverage and PDF text round-trip checks; physical pages 2/176/177 are explicitly empty, and one disagreement on p57 remains for review. Samples and whole-book OCR evidence are recorded in `docs/evals/mineru-ocr-migration-20260926.json`. After the user limited translation testing to a sample, one three-page pdf2zh run completed in 101.97 seconds. Visual review found intact body paragraphs and preserved table images, but the unchanged 0.30 gate rejected median 0.298555 (0/3 near-zero pages). The six-page bilingual candidate remains staged; no canonical translation was published, no provider replay was made, and no whole-book translation was run.
  - Keep pdf2zh provider progress on stderr so strict CLI stdout contains only its JSON receipt. The live sample exposed mixed progress/receipt output; 74 backend/CLI tests pass, including a real local subprocess regression.
  - Scope Chinese translation coverage to measurable body pages using exact reference-section bookmarks and bounded, numbered source Notes headings. Keep mixed opening pages and uncertain sections scored; retain the 0.30 median and at-most-10% near-zero policy. Record exclusions and their evidence in existing coverage details, and fall back to unfiltered measurement if too few body pages remain. Read-only BTS remeasurement passes the current Greenspan and Buthe–Mattli PDFs while retaining rejection of the older Buthe–Mattli PDF; excluded notes/references are not claimed to be translated.
  - Validation: 131 translation, ToUnicode and contract tests passed, including both backend transactions persisting exclusions in successful manifests and rejected-generation receipts, mixed-page retention, outline boundaries and the unchanged 10%/0.30 limits. Workflow parity/type checks and maintainer mirrors passed. Real-PDF hashes and section ranges are recorded in `docs/evals/translation-coverage-sections-20260926.json`; BTS PDFs remained byte-identical and no provider was called.

- **0.67.7** (2026-09-25): Require image-bearing translation sources to pass paragraph layout preparation before either provider starts, using the existing recovery PDF path. Share `relayer_page`'s image-object predicate; ABBYY font names do not exempt scans, and born-digital pages retain their text. Embed source-bound layout evidence in the PDF, reject stale/unproven recovery reuse, and fail layout mode on missing MinerU, all-empty grouping or zero actually placed paragraphs instead of substituting per-line/Tesseract output. Ordinary OCR fallback and existing coherent translations remain reusable. Recursively clean committed generation directories, preserve failed/unknown work, and remove temporary locks with waiter inode revalidation to maintain exclusive publication.
  - Validation: 398 extraction, translation, ToUnicode, status, Workflow and contract tests passed, including mixed scan/digital pages, a TimesNewRomanPSMT scan, source-bound recovery reuse, zero actual paragraph placement, nested cleanup, preserved failed fences and a detached-lock waiter race. Workflow parity/type checks and maintainer mirrors passed. Model calls were stubbed; no real whole-book OCR or provider translation was run.

- **0.67.6** (2026-09-25): Clarify that translate-agent's 60-second observation bound is not a total monitoring budget: continue following one identifiable writer for hour-long books, and retain exact reconciliation evidence if the host really forces the invocation to end. Remove Immersive's default 180-check/30-minute cutoff while keeping explicit caller polling limits, terminal/transport failures and interruption. Emit compact progress/heartbeat evidence on stderr. Paid-task creation, generation fences, validation and publication stay with the existing transaction.
  - Validation: 101 translation, ToUnicode and contract tests passed, including a simulated 65-minute task through the strict transaction with one task creation, canonical publication and later reuse; provider/transport failure, interruption and explicit polling limits remain covered. Workflow parity/type checks and maintainer mirrors passed. No hour-long real Host invocation or paid translation was run.

- **0.67.5** (2026-09-25): Constrain both DS OCR2 and MinerU uvx environments to `mlx<0.32` alongside `mlx-vlm==0.3.12`; MLX 0.32 broke paragraph detection on every page while the optional grouper silently degraded. Report actual grouping coverage and explicitly warn when no page returns paragraph blocks. Raise the Chinese translation median gate from 0.22 to 0.30 and reject a near-zero ratio (<0.05) on more than 10% of measurable body pages, even with a healthy median. Low-text plates remain excluded; ToUnicode repair still precedes both checks. Both backends preserve rejected candidates in their generation directory without publishing canonical output, and existing receipt shapes retain the rejection counts and thresholds in detail.
  - Validation: 187 extraction, translation, ToUnicode and contract tests passed; the coverage CLI self-check passed. Both real uvx command environments, with UV_CONSTRAINT removed and offline resolution, selected mlx 0.31.2 / mlx-vlm 0.3.12 and successfully imported the corresponding runtime. No book-length OCR/model inference or provider translation was rerun.

- **0.67.4** (2026-09-24): Inspect complete PDF bytes locally instead of returning a filename after a 2 MiB prefix read and relying on WebFetch. Return bounded physical-page text, total pages, byte identity and labelled PDF metadata; explicitly requested scratch directories retain the PDF and page images for scanned title pages. Local exact-path inspection can revisit selected pages without downloading again, without claiming an unverified remote URL association. Identify can read these exact evidence paths; material publication and canonical identity still belong to the existing Archive Workflow. This addresses the 77 MB Bill Daniels 85/86 guide whose title page was available but never reached Identify.
  - Validation: 372 relevant tests passed; Workflow build/check and maintainer mirrors passed. The real 77,097,980-byte, 1,334-page PDF returned physical page 5 text and a visually verified title-page image; local reinspection of pages 5/19 retained the same source digest and page-5 rendering. Source CLI evidence is recorded in `docs/evals/archive-pdf-inspection-20260924.json`; no new model Host acceptance or formal Archive publication is claimed.

- **0.67.3** (2026-09-24): Recognize existing Archives under one `collection.md`-marked parent without adding membership metadata. Resolve/status/collect preserve exact grouped paths and URL ownership; path changes invalidate stale observations. Coordinate collection moves with Marple through a shared filesystem lock and pending-operation guard, including malformed journals. Extend Topic references and generated Workflow contracts; new Archives still start at the root. Reject ambiguous, nested, duplicate-slug and unsafe routes before writing. No Book or manifest/0.2 format change.
  - Integrate the pending Webpage capture and agent-contract changes: pinned output descriptors, capture/compiler supervisors, bounded WebKit readiness, literal Identify resolver calls, and strict Host evidence validation. Preserve their detailed 0.65.36 acceptance history below; the current release verification is recorded in `docs/ARCHIVE_COLLECTIONS_RELEASE.md`.

- **0.67.2** (2026-09-23): Make snowball research explicit in the standalone Codex Skill. Start from seed sources and notes, follow concrete leads, revisit earlier observations when new evidence changes their meaning, and turn synthesis into further questions. Keep promising and deferred leads in existing notes, choose among branches, and seek independent records beyond the initial source network. User-specified rounds bound connected exploration rather than a predetermined list of archives; CC material reports may return useful leads without expanding their acquisition scope. No new note schema, controller or research quota is introduced.

- **0.67.1** (2026-09-23): Recenter the standalone Codex Skill on source reading, research notes, revisiting observations and developing interpretations that guide further exploration. Move CC launch, routing and handoff mechanics to an on-demand reference while preserving the required Monster command and BTS placement. Research rounds track exploration and judgement updates rather than archive counts; ordinary summaries and single-material outcomes do not end the requested research. Reuse existing Topic prose for notes, keep collection and independent research concurrent, and reserve expanded technical inspection for concrete failures. Packaging and route checks cover the reference split; multi-round research behavior still needs a live research trial.

- **0.67.0** (2026-09-23): Add one standalone Quasi Skill for Codex, depending on Orca orchestration and delegating whole material/draft tasks to Claude Code. Codex owns research and Topic writing; CC retains Workflow execution, exact observations and audit. Dispatched human gates use Orca ask/reply rather than a hidden local question UI. Remove the obsolete Codex plugin manifest so Codex does not install Quasi hooks or expose the Claude skill collection; keep all Claude entrypoints. Require the user's Monster settings and permission flags, with CC launched in the actual project where Quasi is enabled. Verified the handoff in BTS against its installed 0.66.8 plugin: one real Archive audit returned complete, and all five existing material files remained byte-identical.

- **0.66.8** (2026-09-22): Accept required Archive original titles and descriptions in Workflow status observations. The 0.66.7 CLI added these fields, but the closed entry parser still rejected them as material.invalid_input after successful collection. Align the parser with the producer and add a collect-to-status-to-generated-entry regression that reaches Audit with saved originals; blank display metadata remains invalid. Reproduced the failure with the readonly BTS Emergency Charger status and verified the corrected entry reaches Audit without recapture.
...
- **0.66.0** (2026-09-22): Topic research is now a main-agent Skill loop. Delegate independent search lines and bounded synthesis in parallel; main selects candidates, personally reads returned material, drives existing material Workflows and decides the next batch. Remove the active Topic controller bundle, retaining its generated regression reference under deprecated/. No per-material Topic checkpoint, repeated recall, mandatory Steer or card-admission gate. Export the canonical Topic contract for direct writers; keep existing saved pages compatible and assign one writer per shared file. Material validation and continuations remain unchanged.

- **0.65.41** (2026-09-21): Supply the missing canonical Topic artifact contract to outline, evidence-card, overview and resources writers. BTS checkpoint writers had copied request bookkeeping into frontmatter and invented type: topic-outline, leaving readable files unprojectable by quasi-status. Generate the full producer contract from TopicSchema and TOPIC_BODY, bind each output role, and instruct steer to reconcile invalid metadata while retaining user content. Keep strict status admission and unchanged-observation stopping intact. Regression coverage checks all four producer envelopes and sequential Talk/Paper disk projections. Validation: 1,198 tests passed; one actual Claude steer invocation via Orca repaired the isolated BTS outline, retained its body byte-for-byte, returned a coherent complete receipt, and produced valid status with both admitted materials. BTS originals were unchanged.

- **0.65.40** (2026-09-21): Fix Topic Recall canonical-path receipts. BTS Host evidence showed eight legitimate recalled products returned as absolute filesystem paths, which the exact path/slug join correctly rejected as workflow.incoherent_complete. Recall now supplies a project-relative path contract and constrains StructuredOutput paths to canonical vault routes, allowing provider-level correction before consuming the receipt. Source recording/transcript availability is explicitly separate from canonical-product recall eligibility. Null paths, duplicate rejection and exact kind/slug joins remain unchanged; no post-hoc path rewriting or stage replay. Validation: 1,193 local tests passed; a fresh BTS Claude/Orca Host run executed one readonly recall, returned eight exact relative paths, and reached needs_observation for the original Talk without entering writers.

- **0.65.39** (2026-09-21): Archive 原件采集接入 Topic。新增 quasi-archive inspect/collect/read，保存有描述性文件名的网页快照、PDF、组图和直接音视频；archive.md 自由陈列，manifest.yaml 维护共同出处与逐文件来源例外，Marple 按相对路径解析。缺失只记录 coverage，允许链接型空清单，不引入章节结构或转录。旧记录补采保留正文与元数据；同 slug/URL 写入以进程锁和 fresh revision 防止覆盖及丢失 topics，发布结果未知不重放。Schema 0.9.2，新增原件一致性观察及 Topic exact original inputs。
...
- **0.65.36** (2026-09-20): **Webpage capture 等待延迟 DOM，父进程兜底终止挂起的 WebKit，明确 specialist 路径根。**
  - 十个 active agent 明确绝对 ref 原样使用、非空 `CLAUDE_PROJECT_DIR` 优先、空值／未设置回退 specialist cwd；receipt 保留原相对 ref，禁止通过 `cd` 或目录发现替代 exact refs。Literal-ref behavior has deterministic transcript coverage and passed independent fresh Host acceptance on 2026-09-21; historical traces remain separate evidence.
  - 固定 750ms 改为 monotonic bounded DOM/resource stabilization：约 200ms 采样、最短 1.5s、quiet 0.6s、最多 5s；isolated world 计算 URL、readyState、DOM hash/长度、body、resource、images、fonts 的小 fingerprint，Swift 通过随机 token 的 message handler 持有网络 pending Set/revision；quiet 仅在 pending 为零时成立。document-start 不可覆盖的 fetch/XHR hook 捕获原生 intrinsics，fetch 克隆流逐块读到 body 结束才 settle，不在 headers 到达时提前放行。生产不读取测试时序变量，保留 native 60s 总预算和 single-terminal arbiter。
  - Python 使用独立进程组与 65s hard watchdog，超时 TERM/grace/KILL/reap，各段 drain/wait 均有界，即使逃逸子进程持有 pipes 也不会无限等待；返回 `webpage.capture_timeout`，不重试，保留 staging cleanup 和 no-clobber。SIGTERM/SIGINT/SIGHUP 与其它 BaseException 同样有界清理并传播终止语义；Swift 监测 parent death 并退出；独立 capture guardian 覆盖 Python SIGKILL/崩溃后的 staging 清理。
  - Native binary cache keys include Swift source, fixed recipe/deployment target, architecture/platform and bounded compiler version/path identity plus the full explicitly sanitized effective compiler environment (including SDKROOT, DEVELOPER_DIR and target/cache settings). A separate compiler supervisor owns temporary files and process-group cleanup, including caller SIGKILL; failed source write/fsync/compile preserves known-good binaries.
  - Host 验收逐项绑定 provisional→canonical sidecar args/result、resume_seed 与两次 coordinator fresh status，journal 每个 started 必须有唯一同 agentId result；逐行核验 specialist cwd，按 operation owner 绑定 inspect/capture/extract 的 literal URL/ref 和唯一成功 CLI JSON receipt，仅接受 bare shim 的单一 simple command，拒绝 prefix/suffix、替代 executable 与冲突 tool ID；全部 evidence JSON 统一拒绝任意层重复 key 和 NaN/Infinity，最终另行调用源码 `bin/quasi-status` 验盘。Host 使用 project-only settings、禁用 Chrome；默认仍不执行付费 E2E。
  - TaskOutput completion 必须绑定 launch 后唯一真实 TaskOutput call 及唯一较后 non-error result，再绑定 task_id、completed status 与 sidecar result；拒绝 orphan payload、error launch/status 与多 completion channel。CLI receipts 按封闭 key set、类型、摘要、时间戳和 metadata 一致性验证，capture 补齐 requested `url`。
  - 真正跨 document／reload 的主框架导航放行前轮换 Swift generation secret 并重置 readiness；仅 fragment 改变与 `javascript:` action 保留当前 generation，避免没有新 document-start init 时永久失去 readiness。分类比较移除 fragment 后的完整 URL，并保留 reload、表单及 provisional redirect 的安全边界；旧 document 的排队 init/start/settle 无法污染新状态，当前 generation 的重复 init 也不能覆盖 active set。真实 Swift 协议测试与 WebKit 二次导航回归覆盖该 ordering。
  - At the 5s local bound, the last sample must be valid AND archive-safe: ready complete, no incomplete images/loading fonts, WebView not loading, pending zero. Only DOM churn may bypass quiet at the bound. A >5s streamed body fails stabilization without staging/canonical publication; 2.3s streaming and safe continuous churn remain covered.

  - Capture pins project/output directory descriptors with component-wise no-follow opens. Stage allocation, stat/link/unlink and directory fsync share that pinned parent; caller ancestry must still match before publication. A separate `scripts/webpage/capture_guard.py` owns stage creation through commit/abort and removes it after caller death, including allocation and post-helper gaps. A link that takes effect then raises is an unknown outcome, never a safe failure/retry.
  - Host evidence now requires non-null usable identity bound to inspection/capture/extraction, inspect result before its exact resolver item, run-scoped specialist metadata/agent type, and full closed Webpage needs_observation/complete MaterialResult variants. Historical traces remain historical evidence only; the final production contract passed an independent fresh Host run on 2026-09-21.
  - Native success uses the exact Swift protocol without string coercion. Capture inherits the original exclusive staging fd and checks inode identity, so pathname symlink substitution cannot redirect writes. All validation, hashing and receipt construction precede no-clobber commit; postcommit errors are `webpage.capture_outcome_unknown`, requiring fresh disk observation and never writer replay.
  - Host evidence enforces closed Webpage status keys/types/identity/timestamps and one Identify-owned resolver call with its real closed result, binding URL, fresh no-owner, suggested slug and final identity. Specialist status calls and direct Python/module/native writer entrypoints are rejected. The combined contract has deterministic coverage and passed fresh paid Host acceptance on 2026-09-21.

  - Capture root validation now starts with a no-follow component walk from `/` or an already-open cwd, including ancestors above the project root; no validated pathname is reopened as write authority. Added project-ancestor symlink swaps and absolute/relative/empty-root coverage.
  - All Host JSON rejects exponent overflow as well as duplicate keys and NaN/Infinity; TaskOutput completion envelopes follow the closed observed Host ABI. The 5s bound may waive only DOM quiet: network/resource revision quiet remains mandatory, including short fetch/XHR calls that settle between samples. Production loopback regressions cover continuous short-request churn.

  - An earlier fresh Host run reached durable complete but failed Identify evidence after two piped stdin resolver attempts missing title/site. The Identify request now advertises only one direct literal `--items-json` call after successful Inspect, with exact five-field bindings; the specialist contract prohibits pipes/stdin/repetition/repair. Generated-bundle and transcript regressions cover this failure. The deleted vault was not resumed/replayed. The corrected direct `--items-json` contract passed a new fresh Host run with a new disposable vault/session on 2026-09-21: `1 passed in 222.19s`. Acceptance covered the real named `workflows/webpage.mjs`, owner-correct specialists, two Workflow calls and two fresh coordinator statuses, one capture writer, bare literal refs, closed receipts/MaterialResults/metadata, independent source `bin/quasi-status` proving all three artifacts present/usable, and the delayed sentinel in WebMainResource, `source.md` and canonical `## Content`.
...
- **0.65.35** (2026-09-13): **Book Prepare 的 `ocr_required` 回执接受 exact `usable:false` normalized 负面证据；Webpage 输入不再依赖 Workflow 沙箱里并不存在的 `URL` 全局；`quasi-audit` 报告模式严格只读，typecheck 区分 violation 与 advisory，SPEC 同步到 0.8.0。**
  ...
  - predicate 与 Paper 对齐：`artifacts` 为 `[]`，或恰好一条绑定到 request `normalized_document` ref 的 `{exists:true, usable:false}` 负面证据；`selected_source`、`normalized_path`、`manifest_fingerprint`、`mode`、`disposition` 仍须为 null，`chapter_count:0`、`chapters:[]`；legacy 恢复模式与 committed generation 输入仍拒绝 `ocr_required`。`book.prepare` envelope 新增 `disposition_contract`：`ocr_required` 只在 direct PDF 且非 legacy 模式下出现，明说判定成立后不得再做 TOC/pattern/manual split、不得生成、复用或核对 manifest 与 chapter files；`extract-agent` 的 Book Prepare 同步写明即停规则与允许的负面证据形态。
  - 事故形态（selected_source/manifest/mode/chapters 非空 + `usable:false` artifacts + `ocr_required`）仍是 `workflow.incoherent_complete`，且不会启动 `book.ocr`。`quasi-status` 依旧只做结构与非空检查、不判语义可读性：已写到盘上的乱码 manifest 与 chapter 文本会被机械视为 usable 并让 plan 直接进入 fanout，需要人工隔离后重新观察。新增 dispatch 层（负面证据放行、错 path/role、usable:true/null、额外 artifact、事故形态、generation 输入、EPUB 均拒绝）与 plan 层（`book.prepare → book.ocr → needs_observation`；事故形态 incoherent 且不启动 OCR）回归。
  - **Webpage `URL`。**
  - Workflow 沙箱只提供核心 ECMAScript 与宿主自己的 helper，没有 `globalThis.URL`。共享的 `normalizeWebUrl` 此前用 `new URL(...)` 解析，构造器缺席时抛出的 `TypeError` 被 `catch` 折叠成「URL 非法」：Webpage 的 provisional 与 canonical 输入在派出任何 agent 之前一律 `material.invalid_input`，Paper 的 `material.webpage_redirect` 分支把有效 URL 误报为 `workflow.incoherent_complete`，Author 与 Topic 的 Paper 成员重定向因此到不了各自显式的 composition gate。
  - `normalizeWebUrl` 改为只用核心 ECMAScript 的自包含解析器，并按 `scripts/webpage/webarchive.py::normalize_web_url` 产出同一个比较形式：scheme 与 host 小写、去默认端口、丢弃 fragment、空 path 记作 `/`、空 query 不写 `?`，path 与 query 原样保留，不再做 percent-encoding 改写、`.`/`..` 归并或 IDN punycode 转换。既有的更严格前置检查保留（长度上限、控制字符、必须有 `://`、authority 不得含 `@`），并新增拒绝 host 中的 `%`。
  - 测试沙箱与真实沙箱对齐：`tests/workflow_harness.mjs` 不再向 `run-generated` 的 vm context 注入 `URL`，generated bundle 必须不引用任何缺席的 web 平台全局；新增 Webpage provisional 与 Paper webpage redirect 在无 `URL` 语境下的回归，以及 Python 与 Workflow 两侧 normalizer 的一致性用例。
  - **Audit 报告模式只读。** `quasi-audit --report fields|toc|typecheck` 三种显式报告严格只写 stdout：不再刷新 `.quasi/schema.json`，也不再在 `.quasi/temp/audit-typecheck-*/` 落 `typecheck-results.json` 中间文件；默认 writer audit 仍刷新 snapshot，typecheck 结果经 `evaluate_typecheck()` 留在内存。新增 `--report typecheck`：JSON 输出按 `TYPE_REGISTRY` 覆盖全部注册类型，路径不存在返回 2 且不写盘。`field_distribution` 输出按稳定键序排列，toc 报告与 fields 报告共用 `iter_markdown`，排除所有点目录。
  - **Typecheck 诊断分级。** `split_frontmatter` 保持 frontmatter 失败可区分（`missing_frontmatter` / `invalid_yaml` / `frontmatter_not_mapping` / `missing_type`），不再折叠成一个 `no_frontmatter`；`check_body` 把非 strict BodySchema 下的未知 H2 作为 `body_warnings` 单独返回，audit 以 `status:"advisory"` 计入独立的 `advisory` 计数而非 violation；alias 在 required 检查之前解析。`run_typecheck(write_report=False)` 是严格不写盘路径，独立命令仍写 `.quasi/audit/` 下的具名报告。
  - **SPEC 0.8.0。** `scripts/schemas/SPEC.md` 与 `scripts/schemas/*.py` 的 Pydantic V2 executable schema 同步：11 个 canonical type（含 `webpage`）由 `TYPE_REGISTRY` 派生，BodySchema 表以 `body.py` 为准，Zod 伪代码只作阅读辅助，二者冲突时以 executable schema 为准并同步文档。新增 audit 只读合同、typecheck 合同与 schema snapshot（覆盖全部注册类型、原子且幂等写入）三组测试。
...
- **0.65.13** (2026-08-16): **Workflow 不再依赖 Node 宿主；Book 改名与章节恢复先回到磁盘事实。**
  ...
  - year recanonicalization 继续在同一次 invocation 内把用户选中的 fenced candidate 交给 Acquire，不重新下载；若 Search 找到既有 owner 则立即复用，否则 Acquire 完成后在 Prepare/Analyse 前观察新 route。生成 bundle 的 ABI 测试改在没有 `process` 的 VM realm 中执行，防止 Node 测试宿主再次掩盖 native runtime 依赖。
