...
- Every Stage-owned gate carries a host-owned `material_key` copied from the stamped Stage receipt. For gates resumed through `UserDecision`, Skills and higher-order plans copy that key byte-for-byte and never derive it from identity, material result, or child route. This applies to identity-conflict, Book-year, Book-structure, and Translation-source. Every leaf gate also returns the leaf-owned effective `{route,seed,options}` continuation. Translation-configuration is the sole out-of-band configuration gate: after Configure changes, rerun that returned continuation with fresh status and no decision. Topic plan-level gates remain separately specified.
...
Paper hints require at least one of `title|doi`; Book hints require at least one of `title|isbn`. Optional values are validated only when present. The Skill owns the provisional key: retain a valid caller key when one exists; otherwise use `request-{kind}-{one-based original input ordinal}` for this request. A pre-Search gate's continuation retains that provisional seed; any later gate returns the canonical route and effective seed established by the leaf. The original request key remains only the outer batch's order/presentation identity and never participates in canonical slug derivation.
