...
> **proofread-agent 和 citation-agent 是 Pattern C**: 它们做 LLM 工作(in-place edit / 校引用判断),不调 bin。
...
| `citation-agent` | — | **C** | 纯 LLM 校 draft 引用 + 主题契合判断;skill 走 `quasi-helpers citation *` 做 setup/teardown |
...
|---|---|---|
| **`/quasi:wrap-up`** | `audit-agent` (Phase 0) · `proofread-agent` × 节 · `citation-agent` × 批 | `quasi-helpers proofread {split\|init\|cleanup}` · `quasi-helpers citation {parse\|resolve\|render\|emit-bib}` |
...
Phase 6:  citation-agent × 批              [map]      并行,校对引用
...
### 不动
- `quasi-translate`
- `analyse-agent` (仅英式改名) · `download-agent` · `extract-agent` · `translate-agent` · `proofread-agent` · `citation-agent`
