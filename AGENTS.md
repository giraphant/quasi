...
- `run-stage` resolves one descriptor row, composes its prompt and schema, invokes exactly one specialist, and returns the host-validated receipt verbatim. It does not route later stages, maintain material state, retry, join, or run a graph.
