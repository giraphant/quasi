...
- Paper and Book OCR share one source/profile-bound immutable generation transaction. Each invocation advances at most one profile-owned page range — 16 pages for `dsocr2-text`, 32 for `tesseract-text` — then returns `needs_observation`; DS OCR2 may fall back to Tesseract inside that same 16-page range and records the actual engine. Fixed legacy Paper artifacts stay read-only, while an already-started fixed Book progress file may finish only through its explicit legacy recovery path. No path permits a background OCR process, hidden cursor, or blind replay.
...
- Both translate backends share one output contract (`processing/translations/{slug}-{canonical-target-lower}.pdf`, with Mainland Chinese canonicalized to `-zh.pdf`; alternating original/translated pages, bookmarks) and both must run `tounicode.py::repair_pdf` before the per-page-median coverage gate. Provider adapters map canonical `zh` to `zh-CN`. Rejected or uncertain generations stay in their fenced `processing/translations/.{stem}.translate-*` directory and never become canonical output.
