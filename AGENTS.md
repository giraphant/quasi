...
- `vault/` holds user-facing reading outputs; `sources/` holds accepted source files; `processing/chapters/`, `processing/translations/`, `processing/talks/`, and `processing/webpages/` hold user-inspectable intermediates; `.quasi/` holds orchestration state, manifests, caches, audit output, and temp downloads. Webpage owns `vault/webpages/{slug}/snapshot.webarchive`, `processing/webpages/{slug}/source.md`, and `vault/webpages/{slug}/webpage.md`. Temp JSON passed to helpers lives under `.quasi/temp/` unless a specific helper contract says otherwise.
