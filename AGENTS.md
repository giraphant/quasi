...
- Skills drive. For Paper, Book, Talk, and Translation, `collect-material` observes one exact status, selects the fixed named entry by kind, consumes one material-level result, presents a typed gate when needed, and re-observes exact artifacts after completion. Webpage is the sole initial exception: Collect transports its exact public URL with `observation:null`, then observes the canonical Webpage route returned by that entry before resuming. One named invocation owns one logical material; only Book may use the host `pipeline()` internally for disjoint chapter outputs. Author and Topic have named composition entries: they return exact child or Topic routes for host observation, then resume from that fresh testimony without rediscovery. `research-topic` only transports the Topic envelope, typed gates, and exact observations.
...
quasi-status --kind paper|book|talk|author|topic|webpage --slug SLUG --json
quasi-status --kind translation --slug SLUG --target-language TAG --json
quasi-status --scan --json
workflows/paper.mjs | book.mjs | talk.mjs | translation.mjs | author.mjs | topic.mjs | webpage.mjs
